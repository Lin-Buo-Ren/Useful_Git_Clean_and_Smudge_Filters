...
main() {
	if [ $PROGRAM_ARGUMENT_ORIGINAL_NUMBER -ne 1 ]; then
		...
		local temp_file_name="$(basename "$temp_file").temporary.stdin" # Argument list is single filename
		local temp_file="${PROGRAM_DIRECTORY}/$temp_file_name"

		cat "$temp_file" | "$PROGRAM_DIRECTORY"/clean-html.bash >"$temp_file"
		mv --force "$temp_file_name" "$temp_file"
		exit 0
	fi
	# Shouldn't be here
}
